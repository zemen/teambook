 ...
 * Добавим новый сток S' и исток T'. Заменим ребро (u, v, l, r) LR-сети 
 * на ребра (u, T', l), (S', v, l), (u, v, r - l).
 * Добавим ребро (T, S, k). Ставим значение k=inf, пускаем поток.
 * Проверяем, что все ребра из S' насыщены(иначе ответ не существует).
 * Бинпоиском находим наименьшее k, что величина потока не изменится.
 * Это k - величина МИНИМАЛЬНОГО потока, удовлетворяющего ограничениям.
 */
// END_CODE
